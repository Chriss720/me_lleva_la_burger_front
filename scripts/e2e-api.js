...
(async () => {
  ...
  try {
    ...
    const token = loginData.token || (loginResp.data && loginResp.data.token);
    ...
    console.log('  Cart items count:', (cartAfter && (cartAfter.items || cartAfter.cartProducts || cartAfter.cart_productos || [])).length);
    ...
    // Mark E2E todo as completed by printing instruction for assistant to update (will update via tool)
    process.exit(0);
  } catch (err) {
    console.error('\nE2E script failed:');
    if (err.response) {
